...
class Region(Struct):
    ...
    def delete_groups(self, digs):
        ...
        self.all_vertices = nm.unique(nm.r_[list(self.vertices.values())])
    ...
    def delete_zero_faces(self, eps=1e-14):
        ...
        for ig, faces in self.faces.items():
            fav = fa.facets[faces]
    ...
    def iter_cells(self):
        ii = 0
        for ig, cells in self.cells.items():
            for iel in cells:
                yield ig, ii, iel
                ii += 1
    ...
    def get_edge_graph(self):
        ...
        rows, cols, vals = [], [], []
        for ig, edges in self.edges.items():
