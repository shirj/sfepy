...
def _get_signed_orientation(ori, ori_map):
    ...
    """
    i_from = nm.array(list(ori_map.keys()))
    i_to = [ii[-1] for ii in list(ori_map.values())]
...
def _permute_facets(facets, ori, ori_map):
    ...
    """
    assert_((in1d(nm.unique(ori), list(ori_map.keys()))).all())

    permuted_facets = facets.copy()

    for key, ori_map in ori_map.items():
        perm = ori_map[1]
...
def get_facet_dof_permutations(int_coors, ori_maps):
    ...
    for ig, ori_map in ori_maps.items():
        dof_perms[ig] = {}
...
class Facets(Struct):
    ...
    def from_domain(domain, kind):
        ...
        if kind == 'edges':
            n_obj = [group.shape.n_edge_total for group in groups.values()]
            nn = 2
        else:
            n_obj = [group.shape.n_face_total for group in groups.values()]
            nn = 4
    ...
    def __init__(self, name, kind, domain, single_facets,
        ...
        self.n_fps = {}
        for ig, facet in self.single_facets.items():
    ...
    def get_dof_orientation_maps(self, nodes):
        ...
        if int_coors.shape[1] <= 3: # Simplex facet.
            ...
            ori_maps = {}
            for ig, ori_map in self.ori_maps.items():
                ori_maps[ig] = {}
                for key in ori_map.keys():
