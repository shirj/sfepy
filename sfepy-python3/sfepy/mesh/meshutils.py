...
class bound:
    ...
    def associateelements(self,elements):
        if self.elementsassociated:
            return
        keys=[k for k in list(self.f.keys()) if k>mshmodelnum]
        for key in keys:
    ...
    def associateelements_key_fast(self,key,elements):
        ...
        listout=list(range(first,first+len(list)))
        #print listout
        #listout.sort()
        #print listout
        self.f[key]=listout
...
class mesh:
    ...
    def writexda(self,filename,verbose=True,b=None):
        ...
        nels=[len(blocks[t]) for t in list(blocks.keys())]
        map2=[]
        for x in mapping:
            k=[y for y in list(blocks.keys()) if y<x[0]]
            n=0
            for t in k:
                n+=len(blocks[t])
            map2.append(n+x[1]-1)
        ...
        f.write(("%d "*len(list(blocks.keys())))%tuple(blocks.keys())+
    ...
    def readpmd(self,filename):
        ...
        if len(x)>1: #old format....
            ...
            def ev1(_str,loc,toks):
                out= list(range(int(toks[0]),int(toks[2])+1))
            ...
            if "Z" in list(tokens["XY"].keys()):
                Zn=list(map(float,tokens["XY"]["Z"]))
                self.is2d=False
            else:
                Zn=[0.0]*len(tokens["XY"]["X"])

            self.nodes=list(zip(list(range(1,len(Zn)+1)),
                list(map(float,tokens["XY"]["X"])),
                list(map(float,tokens["XY"]["Y"])),
                Zn))
    ...
    def readGMV(self,filename,what=2):
        ...
        if what in [0,2]: 
            ...
            check(len(zs),nnod)
            nodes=list(zip(list(range(1,nnod+1)),xs,ys,zs))
        l=f.readline(); 
        check(l,"\n")
        l=f.readline(); 
        x=l.split()
